...
``translate`` ``gffattr``  originally added in https://github.com/ctSkennerton/bioawk and ``bawk`` script added here is ``bioawk_cas -c fastx "$@"``. Optional arg added to this version of gffattr. Also added ``gtfattr`` to parse the slightly different syntax of those attrbute fields.
...
This field has subfields delimited by semi-colons where each subfield has a name and a value after an equal sign (space for gtfattr).
